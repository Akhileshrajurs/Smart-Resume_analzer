...
def delete_all_resume_submissions():
    """Delete all resume submissions and related data from the database"""
    conn = get_database_connection()
    cursor = conn.cursor()
    ...
    try:
        ...
        if count_before == 0:
            return {"success": True, "message": "No resume submissions to delete", "deleted_count": 0}
        
        # Delete in order to respect foreign key constraints
        # 1. Delete from resume_skills (has foreign key to resume_data)
        cursor.execute("DELETE FROM resume_skills")
        
        # 2. Delete from resume_analysis (has foreign key to resume_data)
        cursor.execute("DELETE FROM resume_analysis")
        
        # 3. Delete from ai_analysis if table exists (has foreign key to resume_data)
        cursor.execute("""
            SELECT name FROM sqlite_master WHERE type='table' AND name='ai_analysis'
        """)
        if cursor.fetchone():
        ...
        # 4. Delete from resume_data (main table)
        ...
        return {
            "success": True, 
            "message": f"Successfully deleted {count_before} resume submission(s) and all related data",
            "deleted_count": count_before
        }
    except Exception as e:
        conn.rollback()
        print(f"Error deleting resume submissions: {e}")
        return {
            "success": False, 
            "message": f"Error deleting resume submissions: {str(e)}",
            "deleted_count": 0
        }
    finally:
        conn.close()