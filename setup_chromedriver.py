...
def _handle_error(message, exit_on_error=True):
    """Handle errors according to the requested behaviour.

    - If `exit_on_error` is True: raise RuntimeError(message).
    - If `exit_on_error` is False: print the message and return None (useful for debug mode).
    """
    if exit_on_error:
...
def main(exit_on_error=True):
    ...
    Parameters:
    - exit_on_error (bool): if True, errors will raise exceptions (and be turned into non-zero exit codes
      by the caller). If False, the function will print error messages and return for interactive debugging.
    """
    ...
    if not chrome_version:
        return _handle_error("Could not detect Chrome version. Please install Chrome or Chromium.", exit_on_error)
    ...
    if not chromedriver_path:
        return _handle_error("Failed to install chromedriver.", exit_on_error)
    ...
    try:
        ...
        return _handle_error(f"Error testing chromedriver: {str(e)}", exit_on_error)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Install matching chromedriver for the local Chrome/Chromium installation")
    parser.add_argument('--no-exit', '--debug', action='store_true', dest='no_exit',
                        help='Do not exit on error; print errors and return instead (useful for debugging)')
    args = parser.parse_args()

    try:
        main(exit_on_error=not args.no_exit)
    except Exception as e:
        # If we're in debug/no-exit mode the exception will be printed above and we shouldn't re-exit.
        print(f"Error: {e}")
        if args.no_exit:
            print("Debug mode: not exiting (would have exited with error code 1).")
        else:
            sys.exit(1)