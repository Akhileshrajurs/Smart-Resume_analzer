...
class AIResumeAnalyzer:
    def __init__(self):
        # Load environment variables - check both utils/ and root directory
        utils_env_path = os.path.join(os.path.dirname(__file__), '.env')
        root_env_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), '.env')
        
        # Try loading from utils/ first, then root
    ...
    def analyze_resume_with_anthropic(self, resume_text, job_description=None, job_role=None):
        """Analyze resume using Anthropic Claude via OpenRouter API"""
        if not resume_text:
            return {"error": "Resume text is required for analysis."}
        ...
        try:
            # Prepare the prompt similar to Gemini
            base_prompt = f"""
            You are an expert resume analyst with deep knowledge of industry standards, job requirements, and hiring practices across various fields. Your task is to provide a comprehensive, detailed analysis of the resume provided.
            ...
            # Call OpenRouter API for Claude
            headers = {
            ...
            if response.status_code != 200:
                return {"error": f"OpenRouter API request failed: {response.text}"}
            
            result = response.json()
            analysis = result.get("choices", [{}])[0].get("message", {}).get("content", "")
            model_used = result.get("model", "Anthropic Claude")
