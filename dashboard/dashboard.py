...
from config.database import get_database_connection, delete_all_resume_submissions
import io
import uuid
    ...
    def render_resume_data_section(self):
        ...
        if resume_data:
            ...
            with st.container():
                ...
                with col3:
                    # Delete all submissions button with confirmation
                    st.markdown("<br>", unsafe_allow_html=True)  # Add spacing
                    if st.button("🗑️ Delete All Submissions", type="secondary", key="delete_all_submissions"):
                        st.session_state['show_delete_confirmation'] = True
                    
                    # Confirmation dialog
                    if st.session_state.get('show_delete_confirmation', False):
                        st.warning("⚠️ **Warning:** This will permanently delete ALL resume submissions and related data. This action cannot be undone!")
                        confirm_col1, confirm_col2 = st.columns(2)
                        with confirm_col1:
                            if st.button("✅ Confirm Delete", type="primary", key="confirm_delete"):
                                result = delete_all_resume_submissions()
                                if result['success']:
